...
- If stored data changes shape (a column or table renamed, removed or retyped), increase "schemaVersion",
  ...
  shape. Never edit an existing migration or change "id". Migration steps cannot use TODAY() or USER():
  every copy must upgrade to the same data, so copies edited by different people can be combined.

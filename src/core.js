...
(function () {
  ...
  function rawValue(t, row, key) {
    ...
    if (col && col.type === 'computed') {
      try { return col.fn(row, makeApi()); } catch (e) { console.warn(`[carryall] computed ${t}.${key} failed`, e); return NaN; }
    }
    return row[key];
  }
  ...
  function makeApi() {
    return {
      ...
      openProperties,
      rows: tableRows,
      row: (t, id) => tableRows(t).find(r => r.id === id) || null,
      value: rawValue,
      display: displayOf,
      format: formatCell,
      formatAs,
  ...
  function renderTableView(el, view, api, key) {
    ...
    function refresh() {
      ...
      if (vs.sort) {
        const { key: sk, dir } = vs.sort, m = dir === 'desc' ? -1 : 1;
        rows = rows.map(r => [sortKey(t, r, sk), r]).sort(([a], [b]) => (a == null || a === '') ? 1 : (b == null || b === '') ? -1 : (a < b ? -m : a > b ? m : 0)).map(x => x[1]);
      }
  ...
  function renderSummary(el, view, api) {
    ...
    else if (col.type === 'date' || view.sortBy === 'label') list.sort((a, b) => a.key < b.key ? -1 : a.key > b.key ? 1 : 0);
  ...
  function render() {
    ...
    root.replaceChildren(
      h('a', { class: 'ca-skip', href: '#ca-main', onclick: e => { e.preventDefault(); document.getElementById('ca-main')?.focus(); } }, 'Skip to content'),
      renderHeader(), renderBanners(), S.data ? renderTabs() : '',
      h('main', { class: 'ca-main', id: 'ca-main', tabindex: '-1' }, panel), renderFooter());
    if (focusKey) root.querySelector(`[data-ca-focus="${focusKey}"]`)?.focus();
    document.title = `${S.dirty ? '• ' : ''}${S.fileName ? S.fileName + ' – ' : ''}${S.app.name}`;
  }
  ...
  function renderBanners() {
    ...
    if (S.app.home && !S.isHome && !S.homeBannerDismissed) {
    ...
    try {
      ...
      for (const fx of app.fixtures || []) {
        try {
          ...
          ok(`fixture "${fx.name}" migrates cleanly`, !bad, bad ? `${bad} bad values` : '');
          if (fx.expect) ok(`fixture "${fx.name}" expectations`, fx.expect(d, makeApi()) !== false);
